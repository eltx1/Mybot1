...
export function filterCompletedTradesBySymbolStartTime(completedTrades = [], symbolStartTimes = {}) {
  if (!Array.isArray(completedTrades) || !completedTrades.length) return [];
  if (!symbolStartTimes || typeof symbolStartTimes !== 'object') return [...completedTrades];

  return completedTrades.filter(trade => {
    if (!trade || typeof trade !== 'object') return false;
    const symbol = String(trade.symbol || '').toUpperCase();
    if (!symbol) return true;
    const rawStart = symbolStartTimes[symbol];
    if (rawStart === null || rawStart === undefined) return true;
