...
- **مولد قواعد بالذكاء الاصطناعي**: يدعم التبديل بين Ollama محليًا و OpenAI لإنشاء قواعد تداول يتم حفظها وتفعيلها مباشرة.
...
| `USE_OLLAMA` | إذا كانت `true` يتم استخدام Ollama المحلي، وإذا `false` يتم الرجوع إلى OpenAI. |
| `OLLAMA_BASE_URL` | رابط Ollama المحلي (افتراضي `http://localhost:11434`). |
| `OLLAMA_MODEL` | اسم نموذج Ollama الافتراضي (افتراضي `qwen3.5:2b`). |
| `OPENAI_API_KEY` | مفتاح واجهة OpenAI (مطلوب فقط عند `USE_OLLAMA=false`). |
| `OPENAI_MODEL` | اسم نموذج OpenAI الافتراضي عند استخدام OpenAI (افتراضي `gpt-4o-mini`). |
...
- `prompt_templates`: تعريفات البرومبتات الأساسية لكل استخدام.
- `prompt_template_versions`: نسخ/إصدارات البرومبتات (system + user + settings + variables).
- `prompt_template_audit`: سجل تدقيق كامل للتعديلات والتنشيط وإعادة الضبط.

## إدارة البرومبتات (Admin)

- رابط لوحة الأدمن: `/admin-prompts.html`
- للوصول:
  1. سجل دخولك في الواجهة الرئيسية بحساب أدمن (إما البريد ضمن `ADMIN_EMAILS` أو الحقل `users.is_admin = 1`).
  2. افتح الرابط أعلاه بنفس المتصفح.
- في حال قاعدة البيانات القديمة لا تحتوي عمود `is_admin` شغّل الملف:
  - `database/2026-03-31_admin_access_migration.sql`
- مثال سريع لترقية مستخدم لأدمن:
  - `CALL grant_admin_by_email('owner@example.com');`
- تستطيع من اللوحة:
  - عرض كل البرومبتات.
  - تعديل system/user prompt.
  - عمل Preview قبل الحفظ.
  - حفظ نسخة جديدة وتفعيلها.
  - تفعيل إصدار سابق أو Reset للإصدار الافتراضي.
  - مراجعة Audit trail.

## التبديل بين OpenAI و Ollama

1. في ملف `.env` اضبط:
   - `USE_OLLAMA=true`
   - `OLLAMA_BASE_URL=http://localhost:11434`
   - `OLLAMA_MODEL=qwen3.5:2b`
2. تأكد أن Ollama يعمل محليًا:
   ```bash
   ollama serve
   ollama pull qwen3.5:2b
   ```
3. إذا أردت الرجوع إلى OpenAI:
   - اجعل `USE_OLLAMA=false`
   - اضبط `OPENAI_API_KEY` و (اختياريًا) `OPENAI_MODEL`.

> النظام يقرأ نفس Prompt Templates من قاعدة البيانات بدون تغيير في الجداول، مع طبقة إلزام JSON إضافية لتحسين دقة النماذج الصغيرة.
